...
def next_state_str_board(current_state, move):
  ...
  # ##BBB## -> ##BBB##
    ...
    current_state_copy[(start_x + end_x) // 2] = (
                          current_state_copy[(start_x+end_x) //2][:start_y]
                  + 'W' + current_state_copy[(start_x+end_x) //2][start_y + 1:])
    # if start_y < end_y: # bottom
    #   current_state_copy[start_x] = (current_state_copy[:start_y]
    #                         + 'W' + current_state_copy[start_y + 1:])
    #   current_state_copy[end_x] = (
    # current_state_copy[:start_y]
    #                         + 'B' + current_state_copy[start_y + 1:])
    #   current_state_copy[(start_x + end_x) // 2] = (
    # current_state_copy[:start_y]
    #                         + 'W' + current_state_copy[start_y + 1:])
    # else: # top
    #   current_state_copy[start_x] = (current_state_copy[:start_y]
    #                         + 'W' + current_state_copy[start_y + 1:])
    #   current_state_copy[end_x] = (current_state_copy[:start_y]
    #                         + 'B' + current_state_copy[start_y + 1:])
    #   current_state_copy[(start_x + end_x) // 2] = (
    # current_state_copy[:start_y]
    #                         + 'W' + current_state_copy[start_y + 1:])
      ...
      try:
        if root_state[i][j] == 'W':
          if (root_state[i+1][j] =='B' and root_state[i+2][j]=='B'):
            next_state = next_state_str_board(root_state,(i+2,j,i,j)) # bottom
            next_node = {'state' : next_state , 'parent' : root_state }
            child_nodes.append(next_node)
          if (root_state[i-1][j] =='B' and root_state[i-2][j]=='B'):
            next_state = next_state_str_board(root_state,(i-2,j,i,j)) # top
            next_node = {'state' : next_state , 'parent' : root_state }
            child_nodes.append(next_node)
          if (root_state[i][j-1] =='B' and root_state[i][j-2]=='B'):
            next_state = next_state_str_board(root_state,(i,j-2,i,j)) # left
            next_node = {'state' : next_state , 'parent' : root_state }
            child_nodes.append(next_node)
          if (root_state[i][j+2] =='B' and root_state[i][j+1]=='B'):
            next_state = next_state_str_board(root_state,(i,j+2,i,j)) #right
            next_node = {'state' : next_state , 'parent' : root_state }
            child_nodes.append(next_node)
      except:
        pass
...
def path(root_node, visted):
  ...
  while root_node['parent']:
    find_path.append(root_node['state'])
    root_node = visted[root_node['parent']]
  return find_path
def search(new_fringe, initial_visited):
  while new_fringe:
    root_node = new_fringe[0]
    new_fringe = new_fringe[1:]
    if goal_state(root_node['state']):
      return path(root_node,initial_visited)
    initial_visited[root_node['state']] = root_node
    child_nodes = possible_nodes(root_node)
    for child_node in child_nodes:
      if child_node['state'] not in initial_visited:
        new_fringe.append(child_node)
    print(child_nodes)
path = search(fringe,visited)
